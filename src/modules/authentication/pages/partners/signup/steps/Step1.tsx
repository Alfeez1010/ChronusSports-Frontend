...
const Step1 = ({ data }: { data: any }) => {
  ...
  const handleChange = (value: string) => {
    data?.setRegistrationData((prev: any) => ({
      ...prev,
      registerType: value === prev.registerType ? '' : value,
      period: period,
    }));
    setPeriod();
  };
  ...
  const handleNext = () => {
    ...
    if (
      (!period && data.registrationData.registerType !== 'ATLETA') ||
      (period?.id === 0 && data.registrationData.registerType !== 'ATLETA')
    ) {
          ...
          <motion.div layout className="parent">
            <div className="child mt-8">
              <RegisterType
                data={{
                  types: typesOfPeriod,
                  selected: data.registrationData.registerType,
                  handleChange,
                }}
              />
            </div>
              ...
              <>
                <div className="child col-span-full pb-[24px] pt-[30px]">
                  <Select
                    ...
                    data={{ selected: period, setSelected: setPeriod }}
                  />
                </div>
              </>
            )}
