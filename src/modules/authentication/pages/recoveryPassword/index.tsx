...
const RecoveryPassword = () => {
  ...
  const handleSubmit = async (values: RecoveryPassword) => {
    ...
    try {
      ...
      if (response) {
        Toast.success(
          'Recuperação de senha solicitada com sucesso! Por favor verifique o seu e-mail.'
        );
        setSent(true);
      }
    } catch (error: any) {
                ...
                <div className="mt-6">
                  <button
                    ...
                    className={`h-14 rounded-2xl ${
                      sent
                        ? 'bg-[#1c1e3a] text-[#696c83]'
                        : 'text-[#EEEEEE] bg-[linear-gradient(322.68deg,#803EB4_36.47%,#4F25CC_92.31%)]'
                    }  not-italic font-bold text-base w-full`}
                  >
